...
void init(){
    glClearColor(0,0,0,1);          //ClearColor(R,G,B,alpha) for setting background of the window
    glClear(GL_COLOR_BUFFER_BIT);   //Clears bufferbit
    glMatrixMode(GL_PROJECTION);    //Creates matrix mode for projection
    gluOrtho2D(0,640,0,480);        //Used to set Origin (distance from left,dist from right,dist from top,dist from bottom)
}
...
int main(int argc,char* argv[]){
    ...
    glutCreateWindow("Rectangle Pattern");  //Creates winddow with the name of window as the formal argument
